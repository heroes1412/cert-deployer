...
(cd server && GOOS=windows GOARCH=amd64 go build -mod=mod -ldflags="-s -w" -o ../build/windows/cert-server.exe .)

echo "[2/4] Building Cert Server for Linux (build/linux/cert-server)..."
(cd server && GOOS=linux GOARCH=amd64 go build -mod=mod -ldflags="-s -w" -o ../build/linux/cert-server .)

echo ""
echo "[3/4] Building Cert Agent for Windows (build/windows/cert-agent.exe)..."
(cd client && GOOS=windows GOARCH=amd64 go build -mod=mod -ldflags="-s -w" -o ../build/windows/cert-agent.exe .)

echo "[4/4] Building Cert Agent for Linux (build/linux/cert-agent)..."
(cd client && GOOS=linux GOARCH=amd64 go build -mod=mod -ldflags="-s -w" -o ../build/linux/cert-agent .)
