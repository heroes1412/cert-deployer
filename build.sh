...
(cd server && GOOS=windows GOARCH=amd64 go build -o ../build/windows/cert-server.exe main.go)

echo "[2/4] Building Server for Linux (build/linux/cert-server)..."
(cd server && GOOS=linux GOARCH=amd64 go build -o ../build/linux/cert-server main.go)

echo ""
echo "[3/4] Building Client Agent for Windows (build/windows/cert-agent.exe)..."
(cd client && GOOS=windows GOARCH=amd64 go build -o ../build/windows/cert-agent.exe main.go)

echo "[4/4] Building Client Agent for Linux (build/linux/cert-agent)..."
(cd client && GOOS=linux GOARCH=amd64 go build -o ../build/linux/cert-agent main.go)
